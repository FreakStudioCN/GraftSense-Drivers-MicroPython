...
def test_mcp_functions():
    ...
    # 切换A2电平为高，读取A3验证（短接后A3应同步为高）
    mcp.pin(A2_PIN, value=1)
    a3_val = mcp.pin(A3_PIN)  # 读取A3电平（返回True/False）
    print(f"A2 Output Level: HIGH (1) | A3 Read Level: {'HIGH (1)' if a3_val else 'LOW (0)'}")

    # 切换A2电平为低，读取A3验证
    mcp.pin(A2_PIN, value=0)
    a3_val = mcp.pin(A3_PIN)
    print(f"A2 Output Level: LOW (0) | A3 Read Level: {'HIGH (1)' if a3_val else 'LOW (0)'}")
    ...
    # 用虚拟引脚设置A2为高
    a2_vpin.output(val=1)  # 等价于mcp.pin(A2_PIN, mode=0, value=1)
    a3_val = a3_vpin.value()  # 读取A3电平
    print(f"Virtual Pin A2 Output: HIGH (1) | Virtual Pin A3 Read: {'HIGH (1)' if a3_val else 'LOW (0)'}")

    # ---------- 演示3：批量配置/读取寄存器（进阶） ----------
    print("\n=== Demo 3: Batch Register Operation ===")
    # 读取PortA（A0-A7）的整体模式（IODIR寄存器）
    porta_mode = mcp.porta.mode
    print(f"PortA Mode Register Value: 0x{porta_mode:02X} (A2=Output(0), A3=Input(1) As Expected)")
    ...
    # 再次设置A2为高，A3读取应为低（因为极性反转）
    mcp.pin(A2_PIN, value=1)
    a3_val = mcp.pin(A3_PIN)
    print(f"A2 Output: HIGH (1) | A3 Inverted Read: {'HIGH (1)' if a3_val else 'LOW (0)'} (Expected LOW)")
...
def loop_verify():
    """
    循环验证A2输出与A3输入的电平同步性，每1秒切换一次A2电平
    Args:
        无
    ...
    Notes:
        循环5次，输出每次的电平匹配结果，需短接A2和A3引脚

    ==========================================

    Cyclically verify the level synchronization between A2 output and A3 input, switch A2 level every 1 second
    Args:
        None
    ...
    Notes:
        Loop 5 times, output the level matching result each time, need to short-circuit A2 and A3 pins
    """
    print("\n=== Loop Verification (Switch A2 Level Every 1 Second) ===")
    print("Short-circuit A2 and A3, observe if A3 synchronizes with A2 level changes...")
    count = 0
    while count < 5:  # 循环5次
        # 切换A2电平
        current_val = count % 2  # 0/1交替
        mcp.pin(A2_PIN, value=current_val)
        # 读取A3
        a3_val = mcp.pin(A3_PIN)
        # 打印结果
        print(f"Round {count+1} - A2 Output: {current_val} | A3 Read: {1 if a3_val else 0} | Matched: {current_val == a3_val}")
        time.sleep(1)
        count += 1
...
time.sleep(3)
print("FreakStudio: MCP23017 I2C Expander Function Test - Short A3 and A2 Pins")
...
# 配置引脚（核心：A2输出，A3输入）
# 配置A2为输出模式（mode=0），初始电平低（value=0）
mcp.pin(A2_PIN, mode=0, value=0)
