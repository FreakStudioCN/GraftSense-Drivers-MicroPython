...
# PCF8591默认I2C地址（可通过A0/A1/A2引脚修改为0x48-0x4F）
PCF8591_ADDR = 0x48
# Pico的I2C控制器ID（0或1）
I2C_ID = 0
...
try:
    # -------------------------- 关键修改：I2C总线初始化与设备扫描 --------------------------
    # 初始化I2C总线，严格对齐指定风格
    i2c_bus = I2C(I2C_ID, scl=Pin(SCL_PIN), sda=Pin(SDA_PIN), freq=I2C_FREQ)
    # 开始扫描I2C总线上的设备
    ...
    # 遍历地址列表初始化目标传感器（PCF8591）
    pcf8591 = None
    for device in devices_list:
        if device == PCF8591_ADDR:
            print("I2c hexadecimal address:", hex(device))
            try:
                # 创建PCF8591实例，基于已初始化的I2C总线
                pcf8591 = PCF8591(address=PCF8591_ADDR, i2c=i2c_bus)  # 关键修改：传入已初始化的I2C总线而非重新创建
                print("PCF8591 initialization successful")
                break
            except Exception as e:
                print(f"PCF8591 Initialization failed: {e}")
                continue
    else:
        raise Exception("No PCF8591 found")
    ...
    # 读取AIN0通道的模拟值（0-255）
    analog_val = pcf8591.analog_read(PCF8591.AIN0)
    # 读取AIN0通道的电压值，基于参考电压计算
    voltage_val = pcf8591.voltage_read(PCF8591.AIN0, REF_VOLTAGE)
    ...
    ch0, ch1, ch2, ch3 = pcf8591.analog_read_all()
    # 打印所有通道的模拟值，便于对比查看
    ...
    pcf8591.analog_write(analog_value)
    ...
    try:
        ...
        while True:
            ...
            v0 = pcf8591.voltage_read(PCF8591.AIN0, REF_VOLTAGE)
            v1 = pcf8591.voltage_read(PCF8591.AIN1, REF_VOLTAGE)
            v2 = pcf8591.voltage_read(PCF8591.AIN2, REF_VOLTAGE)
            v3 = pcf8591.voltage_read(PCF8591.AIN3, REF_VOLTAGE)
