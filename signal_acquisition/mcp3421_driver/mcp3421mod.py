...
class Mcp342X(DeviceEx, ADC, Iterator):
    ...
    _config_reg_mcp3421 = (
        bit_field_info(
            name="RDY", position=range(7, 8), valid_values=None, description=None
        ),  # Этот бит является флагом готовности данных. В режиме чтения этот бит указывает, был ли выходной регистр обновлен новым преобразованием. В режиме однократного преобразования запись этого бита в «1» инициирует новое преобразование.
        bit_field_info(
            name="CH", position=range(5, 7), valid_values=None, description=None
        ),  # (channel) Это биты выбора канала, но они не используются в MCP3421.
        bit_field_info(
            name="CCM", position=range(4, 5), valid_values=range(6), description=None
        ),  # (continue conversion mode) Бит режима преобразования. 1 - режим непрерывного преобразования. 0 - режим однократного преобразования.
        bit_field_info(
            name="SampleRate", position=range(2, 4), valid_values=None, description=None
        ),  # Бит выбора частоты дискретизации. 0 - 240 SPS (12 bit); 1 - 60 SPS (14 bit); 2 - 15 SPS (16 bit); 3 - 3.75 SPS (18 bit)
        bit_field_info(
            name="PGA", position=range(2), valid_values=None, description=None
        ),  # Биты выбора усиления PGA. 0 - 1; 1 - 1/2; 2 - 1/4; 3 - 1/8
    )
    # ответ от АЦП
