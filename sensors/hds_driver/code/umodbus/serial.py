...
class ModbusRTU(Modbus):
    ...
    def __init__(
        ...
        """
        if not isinstance(addr, int) or not 0 <= addr <= 247 or not isinstance(baudrate, int) or baudrate <= 0:
            raise ValueError("addr and baudrate must be valid positive integers")
        super().__init__(
...
class Serial(CommonModbusFunctions):
    ...
    def __init__(
        ...
        """
        if not isinstance(uart_id, int) or uart_id < 0 or not isinstance(baudrate, int) or baudrate <= 0:
            raise ValueError("uart_id and baudrate must be valid positive integers")
    ...
    def _exit_read(self, response: bytearray) -> bool:
        ...
        if not isinstance(response, (bytes, bytearray)):
            raise TypeError("response must be bytes or bytearray")
    ...
    def _send(self, modbus_pdu: bytes, slave_addr: int) -> None:
        ...
        """
        if not isinstance(modbus_pdu, (bytes, bytearray)) or not modbus_pdu or not isinstance(slave_addr, int) or not 0 <= slave_addr <= 247:
            raise ValueError("modbus_pdu and slave_addr are invalid")
    ...
    def _send_receive(self, modbus_pdu: bytes, slave_addr: int, count: bool) -> bytes:
        ...
        """
        if (
            not isinstance(modbus_pdu, (bytes, bytearray))
            or not modbus_pdu
            or not isinstance(slave_addr, int)
            or not 0 <= slave_addr <= 247
            or not isinstance(count, bool)
        ):
            raise ValueError("invalid serial request parameters")
    ...
    def send_response(
        ...
        """
        if not isinstance(slave_addr, int) or not 0 <= slave_addr <= 247 or not isinstance(function_code, int):
            raise ValueError("slave_addr and function_code are invalid")
    ...
    def send_exception_response(self, slave_addr: int, function_code: int, exception_code: int) -> None:
        ...
        """
        if not isinstance(slave_addr, int) or not 0 <= slave_addr <= 247 or not isinstance(exception_code, int):
            raise ValueError("slave_addr and exception_code are invalid")
    ...
    def get_request(self, unit_addr_list: List[int], timeout: Optional[int] = None) -> Union[Request, None]:
        ...
        """
        if not isinstance(unit_addr_list, (list, tuple)) or not unit_addr_list:
            raise ValueError("unit_addr_list must be a non-empty sequence")
