...
class MCP9808(object):
    ...
    def _send(self, buf) -> None:
        ...
        支持传入整数（单个寄存器地址）或 bytes/bytearray（寄存器地址 + 数据）。

        Args:
            buf: 要发送的数据
                - int: 寄存器地址（自动转为单字节）
                - bytes/bytearray: 寄存器地址后跟写入数据
        ...
        Args:
            buf: Data to send
                - int: Register address (auto-converted to single byte)
                - bytes/bytearray: Register address followed by data
