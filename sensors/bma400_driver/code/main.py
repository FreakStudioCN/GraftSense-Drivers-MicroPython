...
for device in devices_list:
    if device == BMA400_ADDR:
        print("I2c hexadecimal address:", hex(device))
        try:
            bma_sensor = BMA400(i2c=i2c_bus)
            print("BMA400 sensor initialization successful")
            break
        except Exception as e:
            print(f"BMA400 Initialization failed: {e}")
            continue
else:
    raise Exception("No BMA400 found")
...
for power_mode in BMA400.power_mode_values:
    if power_mode == BMA400.SWITCH_TO_SLEEP:
        continue
    if set_sensor_param(bma_sensor, "power_mode", power_mode, BMA400.power_mode_values):
        acc_data, temp = read_sensor_data(bma_sensor)
...
# 恢复正常模式
set_sensor_param(bma_sensor, "power_mode", BMA400.NORMAL_MODE, BMA400.power_mode_values)

# 2. 输出数据率配置演示
print_separator("2. Output Data Rate Configuration Demo")
for odr in BMA400.output_data_rate_values:
    set_sensor_param(bma_sensor, "output_data_rate", odr, BMA400.output_data_rate_values)
    time.sleep(0.5)

# 3. 滤波器带宽配置演示
print_separator("3. Filter Bandwidth Configuration Demo")
for bw in BMA400.filter_bandwidth_values:
    if set_sensor_param(bma_sensor, "filter_bandwidth", bw, BMA400.filter_bandwidth_values):
        print("    Continuous 3 acceleration readings:")
        for _ in range(3):
            acc_data, _ = read_sensor_data(bma_sensor)
            if acc_data:
                accx, accy, accz = acc_data
...
for osr in BMA400.oversampling_rate_values:
    if set_sensor_param(bma_sensor, "oversampling_rate", osr, BMA400.oversampling_rate_values):
        acc_data, _ = read_sensor_data(bma_sensor)
        if acc_data:
            accx, accy, accz = acc_data
...
for acc_range in BMA400.acc_range_values:
    if set_sensor_param(bma_sensor, "acc_range", acc_range, BMA400.acc_range_values):
        acc_data, _ = read_sensor_data(bma_sensor)
        if acc_data:
            accx, accy, accz = acc_data
            print(f"    Acceleration: x={accx:.2f}, y={accy:.2f}, z={accz:.2f} m/s²")
...
for src in BMA400.source_data_registers_values:
    if set_sensor_param(bma_sensor, "source_data_registers", src, BMA400.source_data_registers_values):
        acc_data, _ = read_sensor_data(bma_sensor)
        if acc_data:
            accx, accy, accz = acc_data
            print(f"    Acceleration: x={accx:.2f}, y={accy:.2f}, z={accz:.2f} m/s²")
...
try:
    print(f"Power Mode: {bma_sensor.power_mode}")
    print(f"Output Data Rate: {bma_sensor.output_data_rate}")
    print(f"Filter Bandwidth: {bma_sensor.filter_bandwidth}")
    print(f"Oversampling Rate: {bma_sensor.oversampling_rate}")
    print(f"Acceleration Range: {bma_sensor.acc_range}")
    print(f"Source Data Register: {bma_sensor.source_data_registers}")
