...
# @File    : si1145_lowmem.py
# @Description : SI1145 紫外线/可见光/红外光/接近度传感器驱动（低内存版）
# @License : MIT
...
class SI1145(object):
    """
    SI1145 紫外线/可见光/红外光/接近度传感器驱动类（低内存版，直接使用寄存器地址）
    Attributes:
        _i2c: I2C 总线实例
    ...
    Methods:
        ...
        read_prox: 读取接近度值
    Notes:
        - 低内存版本直接使用寄存器地址数值，不定义命名常量
        - 初始化时自动执行硬件复位和校准加载
    ==========================================
    SI1145 UV/visible/IR/proximity sensor driver (low memory version, uses raw register addresses).
    Attributes:
    ...
    Notes:
        - Low memory version uses raw register address values without named constants
