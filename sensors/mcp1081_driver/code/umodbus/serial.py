...
class ModbusRTU(Modbus):
    ...
    def __init__(
        ...
        """
        if addr is None:
            raise ValueError("addr must not be None")
        super().__init__(
...
class Serial(CommonModbusFunctions):
    ...
    def __init__(
        ...
        """
        if uart_id is None:
            raise ValueError("uart_id must not be None")
    ...
    def _send(self, modbus_pdu: bytes, slave_addr: int) -> None:
        ...
        if modbus_pdu is None:
            raise ValueError("modbus_pdu must not be None")
    ...
    def send_exception_response(self, slave_addr: int, function_code: int, exception_code: int) -> None:
        ...
        if slave_addr is None:
            raise ValueError("slave_addr must not be None")
    ...
    def get_request(self, unit_addr_list: List[int], timeout: Optional[int] = None) -> Union[Request, None]:
        ...
        """
        if unit_addr_list is None:
            raise ValueError("unit_addr_list must not be None")
