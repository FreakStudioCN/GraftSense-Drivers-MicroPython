...
| IN             | 信号输入（PWM） | 物理引脚20（GP15） | 接收Pico的PWM信号，也可选择Pico其他GPIO（如GP12、GP13、GP14，避开GP26-GP28 ADC引脚，避免干扰ADC功能） |
| SPK+           | 扬声器正极接口 | 模块自带扬声器正极端子 / 外接扬声器正极 | 1. 若模块自带扬声器，该引脚已预接，无需额外操作；2. 若需外接扬声器，必须接8Ω阻抗（0.5W-2W功率）的扬声器正极，阻抗不匹配会导致音量异常或模块过载 |
| SPK-           | 扬声器负极接口 | 模块自带扬声器负极端子 / 外接扬声器负极 | 1. 与SPK+配合实现扬声器驱动，正负极不可反接（反接会导致扬声器振膜振动方向相反，虽不会烧毁硬件，但会因相位抵消导致音量显著降低、音质失真，尤其播放低频音调时更明显）；2. 外接扬声器时需确保负极接线牢固，接触不良会导致间歇性无声音 |
